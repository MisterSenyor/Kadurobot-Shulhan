...
# import magdad.ball_tracker as ball_tracker
# import settings
...
from ball_tracker import BallTracker  # Make sure ball_tracker.py is in the same folder

def run_blocking_tracking(ball_handler: ball_cv.BallDetector, linear_stepper_handler):
    tracker = BallTracker()  # Initialize the prediction tracker
    players_offset = 0

    # Set this to the player's Y-coordinate in the transformed image
    PLAYER_ROW_Y = 120  # ← adjust to match your actual row
    #PLAYER_ROW_Y = int(input("Enter the player's row Y-coordinate: "))
    while True:
        frame = ball_handler.get_frame()
        coordinates = ball_handler.run_frame(frame)
        print(f"{coordinates=}")
        ...
        # Update tracker with current ball position
        tracker.update_position(coordinates[0], coordinates[1])

        # Predict where ball will cross the player row
        predicted_x = tracker.predict_x_at_y(PLAYER_ROW_Y)
        if predicted_x is not None:
            print(f"🔮 Predicted X at Y={PLAYER_ROW_Y}: {predicted_x:.1f}")

            # Optional: Convert prediction to your stepper space here

        # Call the movement handler
        players_offset = handle_blocking(linear_stepper_handler, players_offset, coordinates)

        # Optional: show frame with debug line
        # camera_players_offset = ball_handler.plane_length_to_pixels(players_offset)
        # if camera_players_offset is not None:
        #     cv2.line(frame, (0, PLAYER_ROW_Y), (2000, PLAYER_ROW_Y), (0, 255, 0), 2)
        #     cv2.imshow("Main", frame)
...
def tracking_test():
    ...
    # calibration_test(ball_handler, linear_stepper_handler, 50)
    # move_to_mouse_test(ball_handler, linear_stepper_handler)
    run_blocking_tracking(ball_handler, linear_stepper_handler)
    # kick_test(linear_stepper_handler)
